...
class CLEVRTEX(Dataset):
    ...
    def _reindex(self):
        ...
        prefix = f"CLEVRTEXv2_{self.dataset_variant}_"
        ...
        _max = 0
        for img_path in self.basepath.rglob(f'{prefix}??????_????{img_suffix}'):
            img_name_seq = img_path.name.split('_')
            indstr = img_name_seq[2]
            end_suffix = img_name_seq[3]
            msk_suffix = f"{msk_tag}_{end_suffix}"
    ...
    def _variant_subfolder(self):
        return f"clevrtexv2_{self.dataset_variant.lower()}"
    ...
    def __init__(self,
        ...
        if not self.basepath.exists():
            raise DatasetReadError("Path does not exist")
