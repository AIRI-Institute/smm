...
from models.utils import to_rgb_from_tensor, average_ari, iou_and_dice, average_segcover
from models.seg_metrics import Segmentation_Metrics_Calculator


class SlotAttentionMethod(pl.LightningModule):
    def __init__(self, model: nn.Module, datamodule: pl.LightningDataModule, args):
        super().__init__()
        self.model = model
    ...
    def training_step(self, batch, batch_idx):
        ...
        out = self.model.forward(batch_img, is_Train=True)
        logs = {}
    ...
    def sample_images(self, add_masks=False):
        ...
        if add_masks:
            ...
            masks_gt = mask_gt.unsqueeze(1).expand(-1, -1, 3, -1, -1).cpu() / 10.
            out = torch.cat([out, masks_gt], dim=1) # add gt masks
