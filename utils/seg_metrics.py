...
class Segmentation_Metrics_Calculator:
    ...
    def process_mask(self,
        ...
        for pred_idx in torch.unique(torch.masked_select(pred_mask, valid_pred_mask.bool())):
            ...
            iou = torch.sum(pred_ins * gt_bg_mask) / (
                        torch.sum(pred_ins) + torch.sum(gt_bg_mask) - torch.sum(pred_ins * gt_bg_mask) + 1e-6)
            if iou >= 0.5:
    ...
    def get_matched_bg(self,
        ...
        for pred_idx in torch.unique(torch.masked_select(pred_mask, valid_pred_mask.bool())):
            ...
            pred_ins = (pred_mask == pred_idx) * valid_pred_mask
            iou = torch.sum(pred_ins * gt_bg_mask) / (
                        torch.sum(pred_ins) + torch.sum(gt_bg_mask) + 1e-6 - torch.sum(pred_ins * gt_bg_mask))
            if iou >= 0.5:
